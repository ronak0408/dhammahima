...
import yogSutraData from '../../../Json/yogSutra.json';
...
const YogSutra = () => {
  const { yogSutraId } = useParams();
   const navigate = useNavigate();
  const yogSutra = yogSutraData.yogaTexts.find(v => v.id === yogSutraId);

  const handleBookSelect = (yogSutraId) => {
    // Find the book in the related books array
    const book = yogSutra.relatedBooks.find(b => b.id === yogSutraId);
     navigate(`/yogSutra/${book.id}`);
  ...
  if (!yogSutra) {
    return <div>Yog Sutras not found</div>;
  }
  return (
    <>
      <Navbar />
      <div className="min-h-screen bg-gradient-to-br from-yellow-50 to-white text-gray-800 py-10 px-4 sm:px-6 md:px-20 mt-15">
        ...
        <div className="App bg-gradient-to-b from-amber-50 to-amber-100 min-h-screen py-8 px-4">
          <div className="max-w-6xl mx-auto">
            ...
            <BookSection book={yogSutra.book} />
            <AuthorSection author={yogSutra.author} />
            <RelatedBooks books={yogSutra.relatedBooks} onBookSelect={handleBookSelect} />
        ...
        <div className="max-w-6xl mx-auto mt-12 sm:mt-16 bg-white rounded-3xl shadow-md p-6 sm:p-8 md:p-12 space-y-8">
          <ReviewSection category="YogaSutra" bookTitle="The Yoga Sutra"  reviews={reviews}
