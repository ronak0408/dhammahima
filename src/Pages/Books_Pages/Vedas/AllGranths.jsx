...
import granthData from '../../../Json/granths.json';
import ReviewSection from './reviewSection';
import React, { useState } from 'react';

const AllGranths = () => {
  const { granthId } = useParams();
   const navigate = useNavigate();
  const granth = granthData.granthas.find(v => v.id === granthId);

  const handleBookSelect = (granthId) => {
    // Find the book in the related books array
    const book = granth.relatedBooks.find(b => b.id === granthId);
     navigate(`/granth/${book.id}`);
  ...
  if (!granth) {
    return <div>Granths not found</div>;
  }
  return (
    <>
      <Navbar />
      <div className="min-h-screen bg-gradient-to-br from-yellow-50 to-white text-gray-800 py-10 px-4 sm:px-6 md:px-20 mt-15">
        ...
        <div className="App bg-gradient-to-b from-amber-50 to-amber-100 min-h-screen py-8 px-4">
          <div className="max-w-6xl mx-auto">
            ...
            <BookSection book={granth.book} />
            <AuthorSection author={granth.author} />
            <RelatedBooks books={granth.relatedBooks} onBookSelect={handleBookSelect} />
        ...
        <div className="max-w-6xl mx-auto mt-12 sm:mt-16 bg-white rounded-3xl shadow-md p-6 sm:p-8 md:p-12 space-y-8">
          <ReviewSection category="Granth" bookTitle="The Granth"  reviews={reviews}
